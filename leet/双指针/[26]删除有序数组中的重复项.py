...
class Solution:
    def removeDuplicates(self, nums: List[int]) -> int:
        """快慢指针
        1.双指针均从0出发；
        2.如果fast指针指向的数字与slow指针指向的数字不同，则slow指针向右移一位，将所指的值修改为fast指针的值；
        3.不管slow指针是否移动，fast指针均右移一位；
        4.由于数组本身有序，遍历完之后slow包括之前的位置都是不同的元素，返回值为slow+1
        """
        slow, fast = 0, 0
        while fast < len(nums):
